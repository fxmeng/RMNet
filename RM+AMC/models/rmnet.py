...
def ckpt_to_model(ckpt_path,dist_model_type,n_class=1000):
    if ckpt_path is None:
        model = MobileNetV2(n_class=n_class)
        if dist_model_type == 'mobilenetv1':
            model = mobilenetv2_to_mobilenetv1(model)
        return model
    else:
        ckpt = torch.load(ckpt_path,map_location='cpu')
        if 'mobilenetv1' in ckpt_path:
        ...
        if model_type == 'mobilenetv1':
            return ckpt_to_mobilenetv1(ckpt,n_class)
        elif model_type == 'mobilenetv2':
            model = ckpt_to_mobilenetv2(ckpt,n_class)
            if dist_model_type == 'mobilenetv1':
                model = mobilenetv2_to_mobilenetv1(model)
            return model
        else:
